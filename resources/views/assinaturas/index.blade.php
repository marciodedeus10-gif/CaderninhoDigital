...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto">
        ...
        {{-- Exibir assinatura atual se existir e estiver ativa --}}
        ...
        <div class="mb-8">
            <h2 class="text-2xl font-bold text-gray-900 mb-6">Planos Disponíveis</h2>
                ...
                @foreach($planos as $plano)
                    <div class="bg-white rounded-lg shadow-md p-6 border-2 flex flex-col justify-between {{ $assinatura && $assinatura->plano_id == $plano->id ? 'border-blue-500' : 'border-gray-200' }}">
                        ...
                        <div class="mb-6">
                            ...
                            <ul class="text-sm space-y-1">
                                <li class="flex items-center">
                                    <svg class="h-4 w-4 text-green-500 mr-2" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
                        ...
                        @if(!$assinatura || $assinatura->plano_id != $plano->id)
                            @if($plano->nome == 'Bronze')
                                <button type="button" class="w-full bg-gray-400 text-white py-2 px-4 rounded-lg hover:bg-gray-500 transition duration-200 mt-4" onclick="alert('Seu plano não tem suporte');">
                                    Não disponível
                                </button>
                            @else
                                <form action="{{ route('assinaturas.upgrade') }}" method="POST" class="mt-4">
                                    @csrf
